...
def get_gnn_model(model_cfg_yaml):
    with open(model_cfg_yaml) as f:
        parameter_dict = yaml.safe_load(f)
    model = QuantaMind(**parameter_dict).float()
    return model


class QuantaMind(nn.Module):
    """
    ...
    def __init__(
        ...
        method_use_nonlinear_embedding: bool = False
    ) -> None:
        super(QuantaMind, self).__init__()
        ...
        self.Zmax = Zmax
        self.dij_min = dij_min
        self.use_method_embedding = use_method_embedding
        self.method_use_nonlinear_embedding = method_use_nonlinear_embedding
        ...
        if self.use_method_embedding:
            if self.method_use_nonlinear_embedding:
                self.method_embedding = NonlinearElectronicEmbedding(
                    self.num_features, self.num_residual_electron, activation
                )
            else:
                self.method_embedding = ElectronicEmbedding(
                    self.num_features,
                    self.num_residual_electron,
                    activation,
                    is_charge=False,
        ...
        if train_from_scratch or load_from is None:  # random initialization
    ...
    def calculate_distances(
        ...
        cell_offsets_direct: Optional[torch.Tensor] = None
        ...
        if cell_offsets_direct is not None:
            # Only used when running MD simulations
            assert cell is None
            assert cell_offsets is None
            Rj += cell_offsets_direct
        ...
        vij = Rj - Ri
        dij = norm(vij, dim=-1)
        # modify pairwise distance, used to deal with clashed structure where dij is small (<0.7A). 
        # The energy will be unstable so causing problems
        if self.dij_min is not None:
            distance_modifier = torch.ones_like(dij)
            # (-1,)
            distance_modifier = torch.where(dij < self.dij_min, self.dij_min/dij, distance_modifier)
            dij = dij * distance_modifier
            # (-1, 3)
            vij = vij * distance_modifier.view(-1, 1)
        return (dij, vij)
    ...
    def _atomic_properties_dynamic(
        ...
        method_onehot: Optional[torch.Tensor] = None,
    ) -> Tuple[
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        ...
        x = z + q + s
        if self.use_method_embedding:
            method_embedding = self.method_embedding(z, method_onehot, num_batch, batch_seg, electronic_mask)
            x = x + method_embedding
